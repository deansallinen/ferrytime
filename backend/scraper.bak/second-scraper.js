...
const getRouteID = async route => {
  const { data } = await client.query({
    query: getRouteIDbyName,
    variables: { route_name: route[0] }
  });
...
const updateRouteWaits = async ({ route_name, oversize_waits, car_waits }) =>
  client.mutate({
    mutation: addWaits,
    variables: { route_name, oversize_waits, car_waits }
  });

const updateSailingPercentage = async ({
  route_id,
  scheduled_departure,
  percent_full
}) =>
  client.mutate({
    ...
    variables: { route_id, scheduled_departure, percent_full }
  });
